void beatsin_simple_pattern(bool Red, bool Green) {
  ...
  while (millis() < light_turned_on + initial_interval + pattern_interval) {
    ...
    if (Red == true && Green == true) {
      // Left Side
      leds[(posBeat) / 2]  = CHSV(colBeat_Red, 255, 255);
      leds[(posBeat3) / 2]  = CHSV(colBeat_Red, 255, 255);
      // Right Side
      leds[(posBeat) / 2 * (-1) + NUM_LEDS]  = CHSV(colBeat_Green, 255, 255);
      leds[(posBeat3) / 2 * (-1) + NUM_LEDS]  = CHSV(colBeat_Green, 255, 255);
